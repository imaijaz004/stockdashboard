...
def ai_chatbot():
    ...
    client = OpenAI(
        base_url="https://openrouter.ai/api/v1",
        api_key=st.secrets["openai"]["api_key"],  # from secrets.toml
    )
    ...
    # Row with input + analyze button
    col1, col2 = st.columns([2, 1])
    with col1:
        user_input = st.chat_input("Ask me anything about stocks...")

    with col2:
        run_analysis = st.button("📊 Analyse Graph")
    ...
    # Image-based stock chart analysis
    if run_analysis:
        if "chart_image" not in st.session_state:
            st.warning("⚠️ Please generate a stock chart before analyzing.")
        else:
            try:
